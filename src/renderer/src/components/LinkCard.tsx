...
export default function LinkCard({ link }: { link: Link }) {
  ...
  return (
    <>
      <div className="group relative flex flex-col overflow-hidden rounded-xl border border-subtle bg-raised transition hover:-translate-y-0.5 hover:bg-hover">
        <button
          type="button"
        ...
        >
          <div className="relative h-32 w-full overflow-hidden bg-hover">
            ...
            {!link.isRead ? (
              <span
                className="absolute left-2 top-2 h-2 w-2 rounded-full bg-accent"
                title="Unread"
              />
            ) : null}
          </div>
      ...
      {confirming ? (
        <ConfirmDialog
          title="Delete link"
          message="This permanently deletes the link. This action cannot be undone."
          confirmLabel="Delete"
